...
import {
  LayoutDashboard, AlertTriangle, MapPin, MessageSquareText, Bell, Settings,
  LifeBuoy, Heart, FileText, Compass, Users, Truck, Radio, Wrench, BarChart3,
  Activity, ShieldAlert, Building2, Award, GraduationCap, Boxes, Shield, LogOut,
  ChevronLeft, Search, Sun, Moon
} from "lucide-react";
import {
...
const NAV: Record<Role, { label: string; items: { title: string; url: string; icon: any }[] }[]> = {
  ...
  volunteer: [
    ...
    { label: "Account", items: [
      { title: "Profile", url: "/rescue/profile", icon: Settings },
    ]},
  ],
  authority: [
    { label: "Command", items: [
    ...
    { label: "Account", items: [
      { title: "Profile", url: "/authority/profile", icon: Settings },
    ]},
  ],
  admin: [
    { label: "Admin", items: [
...
const ROLE_LABEL: Record<Role, string> = {
  citizen: "Citizen", volunteer: "Volunteer", rescue: "Rescue Team", authority: "Authority", admin: "Admin",
};
...
function ThemeToggle() {
  ...
  const toggle = () => {
    ...
    try { localStorage.setItem("resqnet.theme", next ? "dark" : "light"); } catch {}
  };
  return (
    ...
    <Sidebar collapsible="icon" className="border-r">
        ...
        {sections.map(sec => (
          <SidebarGroup key={sec.label}>
            ...
            <SidebarGroupContent>
              <SidebarMenu>
                {sec.items.map(item => {
                  const active = pathname === item.url || (item.url !== `/${user.role}` && pathname.startsWith(item.url));
                  return (
      ...
      <SidebarFooter className="border-t">
        <div className="flex items-center gap-2 px-1 py-1">
          ...
          {!collapsed && (
            <Button size="icon" variant="ghost" className="h-8 w-8 shrink-0" onClick={handleLogout} aria-label="Logout">
        ...
        <header className="sticky top-0 z-20 flex h-14 items-center gap-3 border-b glass px-4">
          ...
          <div className="hidden md:flex items-center gap-2 flex-1 max-w-md">
            <div className="relative w-full">
              ...
              <Input placeholder="Search incidents, shelters, missions…" className="pl-9 h-9 bg-muted/40 border-0" />
            </div>
          </div>
          <div className="ml-auto flex items-center gap-1.5">
            <Badge variant="outline" className="hidden sm:inline-flex gap-1.5 font-normal border-success/30 text-success bg-success/5">
              <span className="h-1.5 w-1.5 rounded-full bg-success animate-pulse" /> Live
            </Badge>
            <ThemeToggle />
            <Button asChild variant="ghost" size="icon">
              <Link to={`/${user.role}/notifications` as any}><Bell className="h-4 w-4" /></Link>
            </Button>
          </div>
        </header>
