          ...
          <ResponsiveContainer>
            {cfg.type === "line" ? (
              ...
              <PieChart>
                ...
                <Pie
                  data={cfg.data}
                  dataKey={cfg.series?.[0]?.key ?? "value"}
                  nameKey={cfg.xKey ?? "name"}
                  innerRadius={55}
                  outerRadius={90}
                  paddingAngle={2}
                >
  ...
  return (
          ...
          {extraActions?.map((a, i) => {
            ...
            return (
              <Button
                ...
                onClick={a.onClick}
              >
                {Ic && <Ic className="h-4 w-4 mr-1.5" />}
                {a.label}
              </Button>
            );
          })}
    ...
    >
      ...
      <div
        className={cn(
          "grid gap-4",
          sideCards?.length || progressRows ? "lg:grid-cols-[1.6fr_1fr]" : "",
        )}
      >
        {tableCols && tableRows && (
          <Card className="shadow-elegant border-border/60">
            <CardContent className="p-4 md:p-5">
                  ...
                  <thead>
                    <tr className="text-xs uppercase tracking-wider text-muted-foreground">
                      {tableCols.map((c) => (
                        <th
                          key={c.key}
                          className={cn("text-left font-medium px-2 pb-3", c.className)}
                        >
                          {c.label}
                        </th>
                      ))}
              ...
              <div className="flex items-center justify-between mt-4 text-xs text-muted-foreground">
                <span>
                  Showing {tableRows.length} of {tableRows.length} records
                </span>
                    ...
                    {sc.items.map((it, j) => (
                      <li key={j} className="flex items-center justify-between gap-3">
                        ...
                        {it.tone ? (
                          <Badge
                            variant="outline"
                            className={cn("rounded-full font-normal", toneMap[it.tone])}
                          >
                            {it.value}
                          </Badge>
                        ) : (
...
export function PillBadge({ tone = "muted", children }: { tone?: Tone; children: ReactNode }) {
  return (
    <Badge variant="outline" className={cn("rounded-full font-normal capitalize", toneMap[tone])}>
      {children}
    </Badge>
  );
}
