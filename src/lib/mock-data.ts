...
export type IncidentStatus = "reported" | "triaged" | "dispatched" | "en-route" | "on-site" | "resolved" | "escalated";
...
export const demoUsers: DemoUser[] = [
  { id: "u-1", email: "citizen@resqnet.app", password: "demo123", name: "Aarav Sharma", role: "citizen", avatar: "AS", location: "Mumbai, IN" },
  { id: "u-2", email: "volunteer@resqnet.app", password: "demo123", name: "Priya Patel", role: "volunteer", avatar: "PP", location: "Chennai, IN" },
  { id: "u-3", email: "rescue@resqnet.app", password: "demo123", name: "Cmdr. Rohan Mehta", role: "rescue", avatar: "RM", location: "Delhi, IN" },
  { id: "u-4", email: "authority@resqnet.app", password: "demo123", name: "Dr. Anita Rao", role: "authority", avatar: "AR", location: "New Delhi, IN" },
  { id: "u-5", email: "admin@resqnet.app", password: "demo123", name: "System Admin", role: "admin", avatar: "SA", location: "HQ" },
];
...
const cities = ["Mumbai", "Chennai", "Kolkata", "Bengaluru", "Hyderabad", "Delhi", "Pune", "Ahmedabad", "Jaipur", "Kochi"];
const types: EmergencyType[] = ["flood", "cyclone", "earthquake", "fire", "landslide", "medical"];
const statuses: IncidentStatus[] = ["reported", "triaged", "dispatched", "en-route", "on-site", "resolved"];
...
export const incidents: Incident[] = Array.from({ length: 60 }).map((_, i) => {
  ...
  return {
    ...
    status,
    title: titles[type],
    description: "Residents reporting urgent need for assistance. AI triage suggests immediate dispatch.",
...
export const notifications: NotificationItem[] = [
  { id: "n1", type: "sos", title: "New SOS received", body: "Flood emergency reported near Sector 12, Mumbai. AI triage: HIGH.", time: "2m ago", unread: true },
  { id: "n2", type: "dispatched", title: "Rescue team dispatched", body: "Team Alpha-3 en route to CC-2412. ETA 7 minutes.", time: "8m ago", unread: true },
  { id: "n3", type: "shelter", title: "Shelter available", body: "Civic Center 4 has 38 open beds 1.2km away.", time: "22m ago", unread: true },
  { id: "n4", type: "assigned", title: "Volunteer assigned", body: "Priya Patel joined mission Cyclone relief in Chennai.", time: "1h ago", unread: false },
  { id: "n5", type: "escalated", title: "Incident escalated", body: "CC-2407 escalated to national authority due to severity.", time: "2h ago", unread: false },
  { id: "n6", type: "resolved", title: "Incident resolved", body: "CC-2389 marked resolved. 42 citizens assisted.", time: "5h ago", unread: false },
];
...
export function generateAIResponse(question: string): string {
  ...
  if (q.includes("flood")) return "During a flood: move to higher ground immediately, avoid walking or driving through flowing water, disconnect electrical appliances, keep an emergency kit ready with water, food, flashlight, and medication. I can locate the nearest shelter for you — shall I?";
  if (q.includes("cyclone")) return "Cyclone preparation: secure outdoor objects, stock 3 days of water and food, charge devices, identify the strongest interior room, and monitor official advisories. Based on your location, the nearest cyclone shelter is 2.4 km away.";
  if (q.includes("earthquake")) return "Earthquake safety: Drop, Cover, and Hold On. Stay away from windows and heavy furniture. After shaking stops, check for injuries and gas leaks, then evacuate if the building is damaged. I can dispatch a rescue team if needed.";
  if (q.includes("fire")) return "Fire emergency: evacuate immediately, stay low to avoid smoke, do not use elevators, close doors behind you, and call emergency services. I can route the nearest fire response unit to your location.";
  if (q.includes("medical") || q.includes("hospital")) return "I've identified 3 hospitals within 5 km. The closest with trauma care is City General Hospital — 1.8 km, ~6 min by ambulance. Want me to request a medical evacuation?";
  if (q.includes("plan")) return "Here's a starting family emergency plan: 1) Designate a meeting point, 2) Save emergency contacts, 3) Prepare a go-bag (water, food, medication, documents), 4) Practice evacuation drills quarterly, 5) Subscribe to local alerts. I can generate a printable version.";
