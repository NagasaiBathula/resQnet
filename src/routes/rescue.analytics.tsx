...
const teamPerf = ["Alpha","Bravo","Charlie","Delta","Echo","Foxtrot"].map((t, i) => ({
  name: t, missions: 40 + (i*11)%60, avg: 6 + (i*3)%8,
}));
...
export const Route = createFileRoute("/rescue/analytics")({
  ...
  component: () => (
    <FeaturePage
      ...
      stats={[
        { label: "Avg response", value: "8m 42s", sublabel: "−12% vs target", icon: Clock, accent: "success" },
        { label: "Closed today", value: "184", sublabel: "94% within SLA", icon: Target, accent: "primary" },
        { label: "Open SLA breach", value: "3", sublabel: "all critical", icon: BarChart3, accent: "warning" },
        { label: "Outcome score", value: "92", sublabel: "+4 wow", icon: TrendingUp, accent: "info" },
      ]}
      ...
      charts={[
        ...
        { title: "Incidents by region", type: "bar", data: analytics.regional, xKey: "city", series: [{ key: "incidents" }] },
      ]}
      tableTitle="Team performance"
      tableCols={[
        { key: "name", label: "Team", render: r => <span className="font-medium">{r.name} Squad</span> },
        { key: "missions", label: "Missions" },
        { key: "avg", label: "Avg response (min)" },
      ]}
      tableRows={teamPerf}
      sideCards={[
        { title: "Top contributors", items: [
