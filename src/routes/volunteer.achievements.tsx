  ...
  component: () => (
    <AppShell title="Achievements">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        <StatCard label="Tier" value="Silver" sublabel="42 / 75 to Gold" icon={Trophy} accent="warning" />
      ...
      <Card className="mb-4"><CardContent className="p-5">
        ...
        <div className="flex items-center gap-3 text-sm mb-2"><span className="font-medium">42 missions</span><span className="text-muted-foreground">of 75 required</span></div>
      ...
      <Card><CardContent className="p-5">
        <div className="flex items-center justify-between mb-4"><SectionTitle title="Badges"/><Button variant="outline" size="sm" className="rounded-full"><Share2 className="h-4 w-4 mr-1.5"/>Share profile</Button></div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {badges.map((b, i) => (
            <div key={i} className={"rounded-2xl border p-4 text-center transition " + (b.earned ? "bg-gradient-to-br from-primary/10 to-transparent" : "opacity-50")}>
              <div className={"mx-auto h-14 w-14 rounded-2xl grid place-items-center mb-3 " + (b.earned ? "gradient-primary text-white shadow-glow" : "bg-muted text-muted-foreground")}><Award className="h-7 w-7"/></div>
              <div className="font-semibold text-sm">{b.name}</div>
              <div className="text-xs text-muted-foreground mt-0.5">{b.desc}</div>
              <Badge variant="outline" className="rounded-full mt-2 capitalize text-[10px]">{b.rarity}</Badge>
            </div>
          ))}
        </div>
      </CardContent></Card>
