...
export const Route = createFileRoute("/volunteer/incidents")({
  ...
  component: VolunteerIncidentsPage,
});

function VolunteerIncidentsPage() {
  const [incidentsList, setIncidentsList] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectedIncident, setSelectedIncident] = useState<any>(null);
  const [dialogOpen, setDialogOpen] = useState(false);

  const fetchAssignedMissions = () => {
    setLoading(true);
    incidentService
      .getMyIncidents()
      .then((data) => {
      ...
      .catch((err) => {
        console.error("Error loading volunteer missions:", err);
        setLoading(false);
      });
  };
  ...
  return (
    <AppShell title="My active missions">
      <p className="text-muted-foreground -mt-1 mb-6">
        View active relief operations you are assigned to support.
      </p>
      ...
      {loading ? (
        ...
        <Card className="border-border/60">
          <CardContent className="p-8 text-center flex flex-col items-center">
            ...
            <div className="font-semibold text-lg">No active missions</div>
            <p className="text-muted-foreground text-sm mt-1 max-w-sm">
              You are not assigned to support any active emergency relief operations currently.
            </p>
          </CardContent>
        </Card>
      ) : (
      ...
      {selectedIncident && (
        <IncidentDetailsDialog
          ...
          onUpdate={fetchAssignedMissions}
        />
      )}
    </AppShell>
  );
