...
import { incidents, shelters, notifications, analytics } from "@/lib/mock-data";
import { Link } from "@tanstack/react-router";
import {
  AlertTriangle, ShieldAlert, Building2, MessageSquareText, MapPin, CloudRain,
  Sun, ArrowRight, Heart, Sparkles, Activity, ThermometerSun
} from "lucide-react";
  ...
  return (
    ...
    >
          ...
          <CardContent className="p-6 relative grid md:grid-cols-3 gap-6 items-center">
            <div className="md:col-span-2">
              <div className="flex items-center gap-2">
                ...
                <Badge variant="outline" className="rounded-full">Mumbai · Andheri</Badge>
              </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mt-4">
        ...
        <StatCard label="Reports filed" value="2" sublabel="All resolved" icon={Activity} accent="primary" delay={0.2} />
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-4 mt-6">
        ...
        <Card className="lg:col-span-2">
          <CardContent className="p-5">
            <SectionTitle title="Recent emergencies near you" action={<Button asChild size="sm" variant="ghost"><Link to="/citizen/history">View all</Link></Button>} />
            <div className="space-y-2">
              {recent.map(i => {
                const Icon = typeIcon[i.type];
                return (
                  <div key={i.id} className="flex items-center gap-3 rounded-xl border p-3 hover:bg-accent/40 transition">
                    <div className={cn("flex h-10 w-10 items-center justify-center rounded-xl", typeColor[i.type])}>
                      <Icon className="h-5 w-5" />
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-medium truncate">{i.title}</span>
                        <span className="text-xs text-muted-foreground hidden sm:inline">· {i.caseId}</span>
                      </div>
                      <div className="text-xs text-muted-foreground truncate">{i.location} · {i.affectedPeople} affected</div>
                    </div>
