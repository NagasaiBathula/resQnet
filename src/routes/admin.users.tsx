...
import { Users, Shield, UserPlus, Search, Edit3, Trash2, ShieldAlert, Eye, Power, Lock, Plus } from "lucide-react";
...
function AdminUsersPage() {
  ...
  const handleUpdateAuthority = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem("resqnet.token");
      const res = await fetch(`${API_URL}/api/users/authority/${editAuthUser._id || editAuthUser.id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  return (
      ...
      actions={
        activeTab === "authorities" ? (
          ...
          <div className="flex gap-2">
            <Button className="rounded-full shadow-glow bg-success text-white hover:bg-success/90" onClick={() => setCreateVolunteerOpen(true)}>
              <Plus className="h-4 w-4 mr-1.5" /> Add Volunteer
            </Button>
            <Button className="rounded-full shadow-glow bg-primary text-white hover:bg-primary/90" onClick={() => setCreateRescueOpen(true)}>
    ...
    >
      <p className="text-muted-foreground -mt-1 mb-6">Manage all user accounts, roles, access permissions, and authority nodes.</p>
        ...
        <div className="space-y-6">
          ...
          <Card className="shadow-elegant border-border/60">
            <CardContent className="p-5">
              <form onSubmit={handleSearchSubmit} className="grid md:grid-cols-[1.5fr_1fr_auto] gap-3 items-end">
                <div className="space-y-1.5">
                  <label className="text-xs text-muted-foreground font-semibold">Search Accounts</label>
              ...
              {loading ? (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm min-w-[760px]">
                    ...
                    <tbody>
                      {users.map((u) => (
                        <tr key={u._id || u.id} className="border-b last:border-0 hover:bg-accent/40 transition">
                          ...
                          <td className="p-4 text-xs font-medium">{u.district}, {u.state}</td>
                          <td className="p-4">
                            <PillBadge tone={u.status === "approved" ? "success" : u.status === "pending" ? "warning" : "emergency"}>
                              {u.status}
                            </PillBadge>
                          </td>
                          <td className="p-4 text-right">
                            <Button size="icon" variant="ghost" className="h-8 w-8 rounded-lg" onClick={() => setSelectedUser(u) || setDetailsOpen(true)}>
        ...
        <div className="space-y-6">
          ...
          <Card className="shadow-elegant border-border/60">
            <CardContent className="p-0">
              ...
              {loading ? (
                <div className="py-16 text-center text-sm text-muted-foreground">Loading authority accounts...</div>
              ) : authorities.length === 0 ? (
                <div className="py-16 text-center text-sm text-muted-foreground">No authority accounts established yet.</div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-sm min-w-[760px]">
                    <thead>
                    ...
                    <tbody>
                      {authorities.map((a) => (
                        <tr key={a._id || a.id} className="border-b last:border-0 hover:bg-accent/40 transition">
                          ...
                          <td className="p-4 text-xs">{a.district}, {a.state}</td>
                          <td className="p-4">
                            <PillBadge tone={a.authorityStatus === "Active" ? "success" : a.authorityStatus === "Inactive" ? "warning" : "emergency"}>
                              {a.authorityStatus || "Active"}
                            </PillBadge>
                          </td>
                          <td className="p-4 text-right">
                            <div className="flex justify-end gap-1">
                              <Button size="icon" variant="ghost" className="h-8 w-8 rounded-lg" onClick={() => handleEditAuthorityClick(a)} title="Edit Account">
                                <Edit3 className="h-4 w-4 text-primary" />
                              </Button>
                              
                              {a.authorityStatus === "Active" ? (
                                <Button size="icon" variant="ghost" className="h-8 w-8 rounded-lg" onClick={() => handleUpdateStatus(a._id || a.id, { authorityStatus: "Inactive" })} title="Deactivate">
                                  <Power className="h-4 w-4 text-warning" />
                                </Button>
                              ) : (
                                <Button size="icon" variant="ghost" className="h-8 w-8 rounded-lg" onClick={() => handleUpdateStatus(a._id || a.id, { authorityStatus: "Active" })} title="Activate">
                                  <Power className="h-4 w-4 text-success" />
                                </Button>
                              )}

                              <Button size="icon" variant="ghost" className="h-8 w-8 rounded-lg" onClick={() => handleUpdateStatus(a._id || a.id, { authorityStatus: "Suspended" })} title="Suspend">
                                <Lock className="h-4 w-4 text-emergency" />
                              </Button>

                              <Button size="icon" variant="ghost" className="h-8 w-8 rounded-lg" onClick={() => handleDeleteAuthority(a._id || a.id, a.name)} title="Delete">
      ...
      <UserProfileDetails
        ...
        actions={
          selectedUser?.role === "authority" && (
            <Button variant="outline" className="rounded-full" onClick={() => { setDetailsOpen(false); handleEditAuthorityClick(selectedUser); }}>
      ...
      <Dialog open={createAuthOpen} onOpenChange={setCreateAuthOpen}>
        <DialogContent className="max-w-lg rounded-2xl glass-strong border shadow-elegant">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs text-muted-foreground">Create secure authority endpoints for regional monitoring.</DialogDescription>
          </DialogHeader>
          <form onSubmit={handleCreateAuthority} className="space-y-4 py-3 text-sm">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <Label htmlFor="authName">Full Name</Label>
                <Input id="authName" value={authName} onChange={e => setAuthName(e.target.value)} required />
              </div>
              <div className="space-y-1">
                <Label htmlFor="authMobile">Mobile Number</Label>
                <Input id="authMobile" value={authMobile} onChange={e => setAuthMobile(e.target.value)} required />
              </div>
            </div>

            <div className="space-y-1">
              <Label htmlFor="authEmail">Email Address</Label>
              <Input id="authEmail" type="email" value={authEmail} onChange={e => setAuthEmail(e.target.value)} required />
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <Select value={authDepartment} onValueChange={setAuthDepartment}>
                  ...
                  <SelectContent>
                    <SelectItem value="Disaster Management Authority">Disaster Management Authority</SelectItem>
              ...
              <div className="space-y-1">
                ...
                <Input id="authDesig" value={authDesignation} onChange={e => setAuthDesignation(e.target.value)} required />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <Input id="authPw" type="password" value={authPassword} onChange={e => setAuthPassword(e.target.value)} required />
              </div>
              <div className="space-y-1">
                <Label htmlFor="authConfirmPw">Confirm Password</Label>
                <Input id="authConfirmPw" type="password" value={authConfirmPassword} onChange={e => setAuthConfirmPassword(e.target.value)} required />
              </div>
            </div>

            <DialogFooter className="border-t pt-4">
              <Button type="button" variant="ghost" className="rounded-full" onClick={() => setCreateAuthOpen(false)}>
                Cancel
              </Button>
      ...
      <Dialog open={editAuthOpen} onOpenChange={setEditAuthOpen}>
        <DialogContent className="max-w-lg rounded-2xl glass-strong border shadow-elegant">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs text-muted-foreground">Modify active authority properties.</DialogDescription>
          </DialogHeader>
          <form onSubmit={handleUpdateAuthority} className="space-y-4 py-3 text-sm">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <Label htmlFor="editAuthName">Full Name</Label>
                <Input id="editAuthName" value={authName} onChange={e => setAuthName(e.target.value)} required />
              </div>
              <div className="space-y-1">
                <Label htmlFor="editAuthMobile">Mobile Number</Label>
                <Input id="editAuthMobile" value={authMobile} onChange={e => setAuthMobile(e.target.value)} required />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1">
                ...
                <Input id="editAuthDesig" value={authDesignation} onChange={e => setAuthDesignation(e.target.value)} required />
              </div>
            </div>

            <DialogFooter className="border-t pt-4">
              <Button type="button" variant="ghost" className="rounded-full" onClick={() => setEditAuthOpen(false)}>
                Cancel
              </Button>
              <Button type="submit" className="rounded-full shadow-glow">
                Save Changes
