...
import { Activity, Award, Users, Target, MapPin, Clock, ArrowRight, TrendingUp, Heart } from "lucide-react";
      ...
      <div className="grid lg:grid-cols-3 gap-4 mt-6">
        <Card className="lg:col-span-2">
          <CardContent className="p-5">
            <SectionTitle title="Mission feed" action={<Button asChild size="sm" variant="ghost"><Link to="/volunteer/missions">View all</Link></Button>} />
            <div className="space-y-2">
              {available.map(m => {
                const Icon = typeIcon[m.type];
                return (
                  <Link to="/volunteer/active" key={m.id} className="block">
                    <motion.div whileHover={{ x: 2 }} className="rounded-xl border p-4 hover:bg-accent/40 transition">
                      <div className="flex items-start gap-3">
                        <div className={cn("h-11 w-11 rounded-xl flex items-center justify-center shrink-0", typeColor[m.type])}>
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="text-xs text-muted-foreground mt-1 flex gap-3 flex-wrap">
                            <span><MapPin className="h-3 w-3 inline mr-0.5" />{m.location} · {m.distanceKm} km</span>
                            <span><Clock className="h-3 w-3 inline mr-0.5" />{m.estDuration}</span>
                            <span><Users className="h-3 w-3 inline mr-0.5" />{m.volunteersAssigned}/{m.volunteersNeeded}</span>
                          </div>
                        </div>
                        <Button size="sm" className="rounded-full shrink-0">Accept</Button>
                      </div>
                    </motion.div>
                  </Link>
                );
          ...
          <div className="h-60">
            <ResponsiveContainer>
              <LineChart data={analytics.monthlyTrends}>
                ...
                <Line type="monotone" dataKey="resolved" stroke="var(--color-primary)" strokeWidth={2.5} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </CardContent>
