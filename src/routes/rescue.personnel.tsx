...
const personnel = Array.from({ length: 28 }).map((_, i) => ({
  id: `p-${i+1}`,
  code: `RN-${4001 + i}`,
  name: ["Aarav Sharma","Priya Patel","Cmdr. Rohan Mehta","Meera Iyer","Vikram Singh","Sara Thomas","Karan Kapoor","Anjali Roy"][i%8],
  role: ["Field medic","Boat operator","Squad leader","Drone pilot","K9 handler","Logistics","Comms officer","Firefighter"][i%8],
  team: `Alpha-${(i%8)+1}`,
  region: ["Mumbai","Chennai","Delhi","Pune","Kochi","Bengaluru","Hyderabad","Jaipur"][i%8],
  shift: (["Day","Night","Day","Night","On call","Day","Night","Day"])[i%8],
  status: (["on-duty","on-duty","off-duty","training","on-duty","on-call","on-duty","off-duty"] as const)[i%8],
...
export const Route = createFileRoute("/rescue/personnel")({
  ...
  component: () => (
    <FeaturePage
      ...
      stats={[
        { label: "Total personnel", value: "182", sublabel: "across 28 teams", icon: Users, accent: "primary" },
        { label: "On duty", value: "118", sublabel: "65% coverage", icon: Clock, accent: "success" },
      ...
      tableCols={[
        ...
        { key: "status", label: "Status", render: r => <PillBadge tone={r.status === "on-duty" ? "success" : r.status === "training" ? "info" : r.status === "on-call" ? "warning" : "muted"}>{r.status}</PillBadge> },
        { key: "rank", label: "Rank" },
      ]}
      tableRows={personnel}
      sideCards={[
        { title: "Shift coverage", items: [
