...
function SOSPage() {
  ...
  const cancel = () => {
    setActive(false); setStage(0);
    toast.info("SOS cancelled");
  };

  const timeline = [
    { i: ShieldAlert, t: "SOS triggered", d: "Location captured · CC-2517 created", min: stage >= 1 },
    { i: CheckCircle2, t: "AI triage complete", d: "Severity: HIGH · Type: Flood", min: stage >= 2 },
  ...
  return (
    <AppShell title="SOS Center">
      <p className="text-muted-foreground -mt-1 mb-6">One tap connects you to every responder in your area.</p>
      <div className="max-w-4xl mx-auto grid lg:grid-cols-2 gap-6 items-start">
        ...
        <Card className="overflow-hidden border-border/60 relative shadow-elegant">
          ...
          <CardContent className="relative p-8 flex flex-col items-center text-center">
            <Badge variant="outline" className={cn("rounded-full mb-6", active ? "bg-emergency/10 text-emergency border-emergency/30" : "bg-success/10 text-success border-success/20")}>
              <span className={cn("h-1.5 w-1.5 rounded-full mr-1.5 animate-pulse", active ? "bg-emergency" : "bg-success")} />
            ...
            <div className="relative my-4">
              {active && (
                <>
                  ...
                  <span className="absolute inset-0 rounded-full bg-emergency/20 animate-pulse-ring" style={{ animationDelay: "0.6s" }} />
                </>
              )}
              <motion.button
                whileTap={{ scale: 0.95 }}
            ...
            <p className="mt-4 text-sm text-muted-foreground max-w-xs">
              {active ? "Help is on the way. Stay where you are if safe to do so." : "Press once to send your location and request immediate help."}
            </p>

            <div className="mt-6 flex flex-wrap justify-center gap-2">
              <Button variant="outline" className="rounded-full"><Phone className="h-4 w-4 mr-1.5" />Call 112</Button>
              <Button variant="outline" className="rounded-full"><MapPin className="h-4 w-4 mr-1.5" />Share location</Button>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="shadow-elegant">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              ...
              {active && <Badge className="rounded-full bg-emergency/10 text-emergency border-0">CC-2517</Badge>}
            </div>
                  ...
                  <AnimatePresence key={i}>
                    {t.min && (
                      <motion.li initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} className="flex items-start gap-3">
                        <div className={cn("flex h-9 w-9 shrink-0 rounded-full items-center justify-center",
                          i === stage - 1 ? "bg-primary text-primary-foreground shadow-glow" : "bg-success/10 text-success")}>
            ...
            {active && stage >= 3 && (
              <div className="mt-6 rounded-2xl border p-4 bg-gradient-to-br from-primary/5 to-transparent">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-xs uppercase tracking-wider text-muted-foreground">Team en route</div>
