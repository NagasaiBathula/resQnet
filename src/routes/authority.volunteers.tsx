  ...
  return (
    <>
      <FeaturePage
        ...
        stats={[
          { label: "Volunteers", value: volunteers.length.toString(), sublabel: "Registered", icon: Users, accent: "primary" },
          { label: "Active this month", value: volunteers.filter(v => v.status === "approved").length.toString(), icon: Heart, accent: "success" },
          { label: "Certified", value: volunteers.filter(v => v.skills.toLowerCase().includes("aid") || v.skills.toLowerCase().includes("rescue")).length.toString(), sublabel: "first aid + above", icon: Award, accent: "info" },
        ...
        tableCols={[
          ...
          { key: "rating", label: "Rating", render: r => <span className="text-xs">★ {r.rating}</span> },
          { key: "skills", label: "Skills" },
          { key: "status", label: "Status", render: r => <PillBadge tone={r.status === "approved" ? "success" : r.status === "pending" ? "warning" : "emergency"}>{r.status}</PillBadge> },
        ]}
