  ...
  try {
    ...
    if (req.user.role === "authority") {
      if (resource.managedByState !== req.user.state || resource.managedByDistrict !== req.user.district) {
        return res.status(403).json({ message: "Not authorized to view resources outside your jurisdiction" });
      }
    }
...
router.post("/", protect, authorize("authority", "admin"), async (req: AuthenticatedRequest, res) => {
  try {
...
// @access  Private (Authority, Admin)
router.put("/:id", protect, authorize("authority", "admin"), async (req: AuthenticatedRequest, res) => {
  try {
    const { name, type, description, state, district } = req.body;
    ...
    if (req.user.role === "authority") {
      if (resource.managedByState !== req.user.state || resource.managedByDistrict !== req.user.district) {
        return res.status(403).json({ message: "Not authorized to edit resources outside your jurisdiction" });
      }
    }
...
router.put("/:id/status", protect, authorize("rescue", "authority", "admin"), async (req: AuthenticatedRequest, res) => {
  try {
    ...
    if (userRole === "authority") {
      if (resource.managedByState !== req.user.state || resource.managedByDistrict !== req.user.district) {
        return res.status(403).json({ message: "Not authorized to update resources outside your jurisdiction" });
      }
    }
    ...
    if (userRole === "rescue") {
      ...
      if (!resource.assignedIncident) {
        return res.status(403).json({ message: "Rescue teams can only update status of assigned resources" });
      }

      // Verify if rescue team is assigned to the incident
      const incident = await Incident.findById(resource.assignedIncident);
      if (!incident || !incident.assignedRescueTeam || incident.assignedRescueTeam.toString() !== req.user._id.toString()) {
        return res.status(403).json({ message: "Rescue teams can only update resources assigned to their active incidents" });
      }

      // Can only change to "In Use" or "Available"
      if (status !== "In Use" && status !== "Available") {
        return res.status(400).json({ message: "Rescue teams can only mark resources as In Use or Available" });
      }
    }
    ...
    // Log the change
    resource.resourceActivityLog.push({
      action: status === "In Use" ? "Status Updated" : status === "Maintenance" ? "Marked Maintenance" : "Status Updated",
      performedBy: req.user._id,
      performedByRole: req.user.role,
    ...
    res.status(500).json({ message: "Server error updating resource status", error: error.message });
  }
});
...
// @access  Private (Authority, Admin)
router.put("/:id/release", protect, authorize("authority", "admin"), async (req: AuthenticatedRequest, res) => {
  try {
    const resource = await Resource.findById(req.params.id);
    if (!resource) {
      return res.status(404).json({ message: "Resource not found" });
    ...
    if (req.user.role === "authority") {
      if (resource.managedByState !== req.user.state || resource.managedByDistrict !== req.user.district) {
        return res.status(403).json({ message: "Not authorized to release resources outside your jurisdiction" });
      }
    }

    if (!resource.assignedIncident) {
      return res.status(400).json({ message: "Resource is not currently assigned to any incident" });
    }
    ...
    resource.resourceActivityLog.push({
      ...
      performedBy: req.user._id,
      performedByRole: req.user.role,
      notes: incident ? `Manually released from incident ${incident.incidentNumber}.` : "Released from incident.",
      timestamp: new Date()
    });
...
// @access  Private (Authority, Admin)
router.delete("/:id", protect, authorize("authority", "admin"), async (req: AuthenticatedRequest, res) => {
  try {
    const resource = await Resource.findById(req.params.id);
    if (!resource) {
      return res.status(404).json({ message: "Resource not found" });
    ...
    if (req.user.role === "authority") {
      if (resource.managedByState !== req.user.state || resource.managedByDistrict !== req.user.district) {
        return res.status(403).json({ message: "Not authorized to delete resources outside your jurisdiction" });
      }
    }
