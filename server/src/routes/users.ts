...
router.post("/", protect, authorize("admin", "authority"), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const {
      name, email, mobileNumber, password, role, state, district,
      age, gender, skills, availability, volunteerExperience,
      organizationName, employeeId, designation, yearsOfExperience, specialization,
      emergencyContactName, emergencyContactNumber,
      department, authorityStatus
    } = req.body;
    ...
    if (req.user.role === "authority" && !["volunteer", "rescue"].includes(role)) {
      return res.status(403).json({ message: "Authority can only create Volunteers or Rescue Teams" });
    }
    if (req.user.role === "admin" && !["volunteer", "rescue", "authority"].includes(role)) {
      return res.status(403).json({ message: "Admin can only create Volunteers, Rescue Teams, or Authorities" });
    }
...
router.post("/requests", async (req, res) => {
  try {
    const {
      name, email, mobileNumber, password, role, state, district, address,
      age, gender, skills, availability, volunteerExperience,
      organizationName, employeeId, designation, yearsOfExperience, specialization, officialIdDocument,
      emergencyContactName, emergencyContactNumber
    } = req.body;
...
router.get("/requests", protect, authorize("authority", "admin"), async (req: AuthenticatedRequest, res: Response) => {
  try {
...
// @access  Private (Authority & Admin)
router.put("/requests/:id/status", protect, authorize("authority", "admin"), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
...
router.get("/", protect, authorize("admin", "authority"), async (req: AuthenticatedRequest, res: Response) => {
  try {
...
// @access  Private (Admin Only)
router.get("/authorities", protect, authorize("admin"), async (req: AuthenticatedRequest, res: Response) => {
  try {
...
// @access  Private (Admin Only)
router.post("/authority", protect, authorize("admin"), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { name, email, mobileNumber, password, state, district, department, designation } = req.body;

    if (!name || !email || !mobileNumber || !password || !state || !district || !department || !designation) {
      return res.status(400).json({ message: "Please provide all required fields" });
    }
...
// @access  Private (Admin Only)
router.put("/authority/:id", protect, authorize("admin"), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
    const { name, mobileNumber, state, district, department, designation, authorityStatus } = req.body;
...
// @access  Private (Admin Only)
router.delete("/authority/:id", protect, authorize("admin"), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
...
// @access  Private (Admin Only)
router.put("/:id/role-status", protect, authorize("admin"), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
